...
class RuntimeContext:
    ...
    def set_server_args(self, server_args: ServerArgs) -> None:
        ...
        # and a direct install is roleless — ``publish`` assigns the role
        # after this returns, so a stale role never outlives its lifecycle.
        self._overrides_log = []
    ...
    def preserve_config(self):
        ...
        # Value snapshots: an in-scope ``override`` without a republish mutates
        # the SAME bag objects / log list, so reference snapshots would alias
        # the mutated state and leak it past the scope.
